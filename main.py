...
def check_ffmpeg_available() -> bool:
    ...
    if not ff or not fp:
        ...
        return False
    LOGGER.info("ffmpeg: %s  ffprobe: %s", ff, fp)
    return True
...
JOB_TRACKERS: Dict[str, Dict[str, Any]] = {}
...
async def clear_conversation_after_delay(chat_id, delay=Config.CONVERSATION_CLEAR_DELAY):
    ...
    LOGGER.info("Auto-cleared conversation state for chat_id=%s", chat_id)
...
async def admin_filter_func(_, __, message_or_query):
    try:
        ...
        LOGGER.error("admin_filter_func error: %s", e, exc_info=True)
        return False
...
async def probe_media(file_path: str):
    ...
    try:
        ...
        LOGGER.error("probe_media failed for %s: %s", file_path, e)
        return []
...
async def get_media_duration(file_path: str) -> float:
    ...
    try:
        ...
        LOGGER.warning("get_media_duration failed for %s: %s", file_path, e)
        return 0.0
...
async def progress_callback(current, total, message, action):
    ...
    if now - last.get("ts", 0) > 2:
        ...
        try:
            ...
            LOGGER.warning("progress_callback edit error: %s", e)
...
async def run_ffmpeg_with_progress(
    ...
    LOGGER.info("[job %s] Running: %s", job_id, " ".join(args_list))
    ...
    try:
        while True:
            if JOB_TRACKERS.get(job_id, {}).get("cancelled"):
                LOGGER.info("[job %s] Cancel requested – terminating ffmpeg", job_id)
                try:
        ...
        LOGGER.error("[job %s] ffmpeg runtime error: %s\nStderr tail:\n%s", job_id, e, err_text)
    ...
    if rc == 0:
        ...
        LOGGER.error("[job %s] ffmpeg failed rc=%s:\n%s", job_id, rc, err_text)
        raise RuntimeError(f"FFmpeg failed (rc={rc}).\n{err_text}")

    LOGGER.info("[job %s] ffmpeg finished successfully.", job_id)
...
async def build_ffmpeg_args(
    conv: dict,
    input_file: str,
    watermark_local_file: Optional[str],
...
async def dummy_start_cb(client, cb: CallbackQuery):
    await cb.answer()
...
async def start_command(client, message: Message):
    ...
    await message.reply_text(
        "**🎧 Audio Converter Bot**\n\n"
        "Accepts audio/video files or direct HTTP/HTTPS links, converts with optional watermark mixing, and returns the processed file.",
    ...
    if conv:
        ...
        if job_dir and os.path.isdir(job_dir):
            try:
                ...
                LOGGER.error("cleanup job_dir failed: %s", e)
    CANCEL_DOWNLOADS.discard(chat_id)
...
async def handle_media_file(client, message: Message, conv: dict):
    ...
    if user_id != Config.OWNER_ID:
        ...
        if limits["max_jobs_per_day"] > 0:
            ...
            if today_count >= limits["max_jobs_per_day"]:
                await message.reply_text(
                    f"⛔ Daily job limit reached (`{limits['max_jobs_per_day']}` jobs/day). Please wait until tomorrow.",
                    quote=True,
                )
                return
    ...
    try:
        ...
        if "Download Cancelled by User" in str(e):
            LOGGER.info("Download cancelled for chat %s", chat_id)
            try:
...
async def handle_media_url(client, message: Message, conv: dict):
    ...
    try:
        ...
        LOGGER.error("URL download failed: %s", e, exc_info=True)
        try:
            await status_msg.edit_text(f"❌ **Download Failed**\n\n`{e}`")
        except Exception:
            pass
...
async def message_handler_router(client, message: Message):
    ...
    try:
        ...
        LOGGER.error("get_user_conversation error for %s: %s", chat_id, e, exc_info=True)
        conv = None
    ...
    if not conv and (sent_media or is_url):
        ...
        conv = await get_user_conversation(chat_id)
        LOGGER.info("Auto-created session for chat %s, job %s", chat_id, job_id)
    ...
    LOGGER.debug("chat=%s stage=%s", chat_id, stage)
...
async def track_selection_cb(client, cb: CallbackQuery):
    ...
    buttons = [
        [InlineKeyboardButton("✨ Copy AAC Stream (Fastest)", callback_data="format_aac_copy")],
            ...
            if use_wm:
                ...
                if wm_file_id:
                    try:
                        ...
                        LOGGER.error("Watermark download failed: %s", e, exc_info=True)
...
async def ping_server():
    if not Config.ON_HEROKU or not Config.STREAM_URL:
        ...
        return
    LOGGER.info("Pinger started for %s.", Config.STREAM_URL)
    while True:
        await asyncio.sleep(Config.PING_INTERVAL)
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                async with session.get(Config.STREAM_URL) as resp:
                    LOGGER.info("Pinged %s: HTTP %s", Config.STREAM_URL, resp.status)
        except Exception as e:
            LOGGER.warning("Pinger error: %s", e)
...
if __name__ == "__main__":
    async def main_startup_shutdown_logic():
        ...
        LOGGER.info("Bot @%s started.", bot_info.username)
        ...
        await site.start()
        LOGGER.info("Web server on port %s.", Config.PORT)
    ...
    async def shutdown_handler(sig):
        ...
        if tasks:
            LOGGER.info("Cancelling %s tasks…", len(tasks))
            for t in tasks:
                t.cancel()
    ...
    try:
        ...
        try:
            if os.path.isdir(Config.DOWNLOAD_DIR):
                ...
                LOGGER.info("Cleaned up DOWNLOAD_DIR: %s", Config.DOWNLOAD_DIR)
        except Exception as e:
            LOGGER.error("Failed to clean DOWNLOAD_DIR: %s", e)
        if loop.is_running():
            loop.stop()
